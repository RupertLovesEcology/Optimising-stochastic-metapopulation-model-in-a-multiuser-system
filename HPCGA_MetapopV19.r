# GA MetapopV15.R
# Everything from GA Metapop V11, ported to Parallel-R, MPI via SNOWFALL
# RM I think this should work now?? We'll just need to ensure that all functions are returning dataobjects with the expected dimensions
# might also run into issues with the different data structures (list vs data.table vs data.frame) and need to convert 
# fingers crossed!! it behaves
# ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
## The Genetic optimisation algorithm for the hydroecological Metapopulation Model 
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
## Original Author: Rupert Mathwin, October 2023
## Porting By: Scott Anderson, Nov 2023
##
...
library(snow)
library(snowfall)
library(tictoc) # Timing library!
library(rlist)  # to save resolvedFitSolutions list object for error checking
...
tic()
# Force MPI-Cluster. SNOWFALL will default to RMPI if its in the .LIBS path, 
# But we always want MPI for this one
# TODO: System Env. Vars, to pull in the $SLURM_NTASKS for CPU Count
# Slave Outfile MUST be on /scratch, /home or /cluster, and it will be amalgamated from each worker. 
sfInit( parallel=TRUE, cpus=100, type="MPI", slaveOutfile = "JUMBLED_LOGGING_MESS.log")  # RM- I love this naming convention 
stopifnot( sfParallel() == TRUE )
if( sfParallel() )
  cat( "Running in parallel mode on", sfCpus(), "nodes.\n" )
#Ensure that all slave-runners have the libraries loaded.
sfLibrary(snowfall)
...
# Ensure that we load locally AND on the slaves, the extra-functions-file 
source('matrixOperators.r')
sfClusterEval(source('matrixOperators.r'))
toc()

message("Data Load...")
tic()
# Do the initial Data-Load 
startPops <- fread('GA_Metapop_startPops.csv', select = c(2:1001)) # the starting populations         
# hydrology
StayWet <- fread('GA_StayWet_200AridScenarios.csv')
setnames(StayWet, 1, 'Rw')
setcolorder(StayWet, c(names(StayWet)[-1], 'Rw'))
WetDry <- fread('GA_WetDry_200AridScenarios.csv')
setnames(WetDry, 1, 'Rw')
setcolorder(WetDry, c(names(WetDry)[-1], 'Rw'))
# load the wetland data
wetlandMetadata <- fread('GA_wetlandMetaDataV4.csv')
## Populate underlying movement data (site-to-site distance which incorporates site-to-site movement type)
adj.wetlandMovement <- fread('GA_Meta_adjMovement.csv', select=c(2:24)) # removed rownames to keep as data.table
## load our initial solutions 
fitSolutions <- fread('TEMP_GA_LHfirst100solutions.csv', select=c(2:17)) 
fitSolutions <- as.data.frame(fitSolutions)
toc()

message("Custom Function definitions...")
tic()
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
# custom functions
...
# the main metapopulation simulation function. Gets invoked in parallel.
MetaSim <- function(x, input) {
  ...
  # finalising this for the HPC. Metasim will now run for 1 iteration (in parrallel)
  iterations <- 1
  simYears <- 25
  wetlandNum <- nrow(wetlandMetadata)
  # select the hydrological scenario
  minR <- 1 + (x*simYears)
  maxR <- simYears + (x*simYears)
  ...
  ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
  # Create deepcopies 
  fillTS <- duplicate(fillTS.iter, shallow = FALSE)
  stayWetTS <- duplicate(stayWetTS.iter, shallow = FALSE)
  for (iter in 1:iterations) {
    ...
    fillTS.iter <- duplicate(fillTS, shallow = F)
    stayWetTS.iter <- duplicate(stayWetTS, shallow = F)
...
MetaMetaSim <- function(solution) {
  results_list <- vector("list", length = 100) 
  # Now we call the MPI-Do Work, as we are invoking that MetaSim() Function.
  mpi_res <- sfLapply(1:100, function(y) {
    res <- MetaSim(y, solution)
    results_list[[y]] <- res   
    return(results_list[[y]])
  })
  # RM NOTE Must ensure this function is returning the equivalent of 1 row of 87 numerics (this should work??)
   aa <- NULL
  for (i in 1:length(mpi_res)) {
    aa <- rbind(aa, mpi_res[[i]])
  }
  bb <- colSums(aa)
  newRw <- c(aa[1,1:16],bb[17:length(bb)]) # RM is this  returning 83 numerics? What class is newRw when it comes back? is it a list??
  return(newRw)
}
toc()
## Export these functions we have just defined to the workers. sfClusterExportAll()
## catches the entire GlobalEnv - no need to individually export data objects
message("Cluster Export of all Data...")
tic()
sfExportAll() 

# Debug double-check. List the objects on the slaves 
#sfClusterEval(ls())
toc()
...
for (opt in 1:5000) {
  message("Starting Genetic Iteration: ", opt)
  tic()
  if (opt == 1) {
    # RM- OK at this point fitSolutions is not a List it's a dataframe. 
    #  The default behaviour of lapply on a data.frame is to apply by column 
    #  let's try this to feed in rows instead of columns
    fitSolutions <- split(fitSolutions, seq(nrow(fitSolutions))) # split the dataframe to a list by row
    resolvedFitSolutions <- lapply(fitSolutions, MetaMetaSim)
    # fitSolutions <- rbindlist(resolvedFitSolutions)
    fitSolutions <- transpose(setDT(resolvedFitSolutions))
    # returns a data.table() which should be 100 rows and 87 columns (all numerics)
    # the first 16 values in each element (row) should range from -10 to 10 and all subsequent values are integers
  }
  # RM- I've made some changes to hopefully avoid the non-numeric issues and to safely feed the correct objects into
  #         lapply in the correct formats (lists where each index is single vector of Criteria1-16)
  # breed and mutate our children
  breeders <- data.frame(fitSolutions[,1:16]) # adding this step saves the 'breed' function from producing a Lovecraftian horror
  NewPop <- breed(breeders) 
  ...
  NewPop <- split(NewPop, seq(nrow(NewPop))) # but a list is probably safer for the lapply
  
  # Apply 'MetaMetasim' to our children and row-bind the results in parallel. # Order doesn't matter. 
  evaluatedSolutions <- lapply(NewPop, MetaMetaSim) 
  # evaluatedSolutions <- rbindlist(evaluatedSolutions) # RM  this should return a dataframe with 100 rows of 87 numerics? 
  evaluatedSolutions <- transpose(setDT(evaluatedSolutions))
  ...
  unfitSolutions <- rbind(unfitSolutions, evaluatedSolutions[101:nrow(evaluatedSolutions), ])
  evaluatedSolutions <- NULL
  # SA - No way are not saving this giant thing every iteration - thats ~30 Minute check-points, which is just good practice!
  fwrite(fitSolutions, paste0("fitSolutions", opt, ".csv"))
  fwrite(unfitSolutions, paste0("unfitSolutions", opt, ".csv"))
  unfitSolutions <- NULL
  toc()
}
message("Shutting down R-Cluster")
sfStop()
message("Done!")
